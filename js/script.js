document.addEventListener('DOMContentLoaded', () => {
    ...
    allLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (href && href.includes('property-details.html')) {
                // Let the link for property details navigate normally for now
                return;
            }
    ...
    // This is our "Master List" of properties.
    const propertyDatabase = {
        ...
        // ... Add data for the 4 other properties here in the same format
    };

    // This is the master function that decides which dynamic code to run
    function initDynamicPages() {
        const pathname = window.location.pathname;

        if (pathname.includes('/buy.html')) {
            // The filtering logic for the buy page now lives here
            const filterForm = document.getElementById('js-property-filters');
            if (filterForm) {
                // ... (The static filtering logic from the previous step goes here) ...
            }
        } else if (pathname.includes('/property-details.html')) {
            // This is the new logic for the detail page
            ...
            if (property) {
                ...
                // Re-add gallery functionality after the HTML is created
